...
function updateVideoSource(element) {
  ...
  if (!element.querySelector('source') || element.querySelector('source').src !== newSource) {
    // 기존 소스 제거
    element.innerHTML = '';
...
gsap.set(".sc-intro .headline, .sc-intro .img-headline, .sc-intro .price-area", { autoAlpha: 0, scale: 1.1 });
...
document.querySelectorAll('.gallery-item .btn-controls').forEach(button => {
  button.addEventListener('click', function () {
    const videoElement = this.closest('.frame-content')?.querySelector('.inside-item'); // 비디오 요소 선택
...
const zoomSwiper = new Swiper('.zoom-swiper', {
  effect: 'fade',
  speed: 800,
  slidesPerView: 1,
  grabCursor: true,
  ...
  on: {
    slideChange: function () {
      // 현재 슬라이드 인덱스에 맞게 active 클래스를 업데이트
      updateActiveZoomItem(this.activeIndex);

      // prev 버튼 visibility 설정
      const prevButton = document.querySelector('.zoom-swiper .swiper-button-prev');
      ...
      // next 버튼 visibility 설정
      const nextButton = document.querySelector('.zoom-swiper .swiper-button-next');
...
document.querySelectorAll('.zoom-item button').forEach((button) => {
  button.addEventListener('click', function () {
    ...
    // 모든 zoom-item에서 active 클래스를 제거
    zoomItems.forEach(item => item.classList.remove('active'));
    // 클릭한 버튼의 부모 요소에 active 클래스 추가
    const parentZoomItem = this.closest('.zoom-item');
    parentZoomItem.classList.add('active');

    // 클릭한 버튼에 해당하는 슬라이드로 이동
    const zoomIndex = this.getAttribute('data-zoom-index');
    zoomSwiper.slideTo(zoomIndex); // 해당 슬라이드로 이동
  });
});

// 슬라이드 변경 시 active 클래스를 업데이트
function updateActiveZoomItem(index) {
  const zoomItems = document.querySelectorAll('.zoom-item');
  const zoomList = document.querySelector('.zoom-list');
  // active 클래스 제거
  zoomItems.forEach(item => item.classList.remove('active'));
  if (zoomItems[index]) {
    zoomItems[index].classList.add('active');
  }
  const itemWidth = zoomItems[index] ? zoomItems[index].getBoundingClientRect().width : 0;
  console.log(itemWidth)
  const translateXValue = -((index + 1) * itemWidth) + (zoomList.offsetWidth / 2 - itemWidth / 2);
  zoomList.style.transform = `translateX(${translateXValue}px)`;
}
